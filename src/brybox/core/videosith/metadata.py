...
class VideoMetadata:
    ...
    parsed_filename_date: Optional[datetime] = None  # Date from filename if present
...
class MetadataReader:
    ...
    def extract_metadata(self, file_path: Path) -> VideoMetadata:
        ...
        time_offset = self._determine_time_offset(
            timezone, 
            creation_date, 
        ...
        for key in date_keys:
            if key in raw_exif:
                ...
                try:
                    ...
                    logger.warning(f"Failed to parse {key}: {date_str}")
                    continue
    ...
    def _determine_time_offset(
        ...
        parsed_filename_date: Optional[datetime]
    ) -> Optional[int]:
        """
        ...
        if timezone and creation_date:
            try:
                ...
                logger.warning(f"Failed to calculate offset from timezone: {e}")
        ...
        if parsed_filename_date and creation_date:
            ...
            logger.info(f"Calculated offset from filename: {offset_hours} hours")
            return offset_hours
    ...
    def _find_exiftool(self) -> str:
        ...
        raise RuntimeError(
            "exiftool not found. Install exiftool and add to PATH."
        )
