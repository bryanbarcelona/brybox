...
class ImageMagickConverter(ImageConverter):
    ...
    def __init__(self, mogrify_path: Optional[str] = None):
        """
        ...
        try:
            ...
            if result.returncode != 0:
                raise ConversionError(
                    f"ImageMagick conversion failed: {result.stderr}"
                )
            ...
            if not intermediate.exists():
                raise ConversionError(
                    f"ImageMagick did not create expected output: {intermediate}"
                )
            ...
            raise ConversionError(f"Conversion timed out after 30 seconds")
    ...
    def _find_mogrify(self) -> str:
        ...
        raise RuntimeError(
            "ImageMagick not found. Install ImageMagick 6 or 7."
        )