...
def _load_audiora_config(config_path: Optional[str] = None,
...
class AudioraCore:
    ...
    def process(self) -> _ProcessingContext:
        ...
        # Extract dates
        context.metadata_date = self.metadata_extractor.extract_media_created_date(
            self.audio_filepath
        )
        context.filename_date = self.metadata_extractor.extract_filename_date(
            filename_without_ext
        )
        
        # Validate and choose date
        context.validated_date = self.metadata_extractor.validate_dates(
            context.metadata_date,
            context.filename_date,
            filepath.name
        )
        
        # Extract session name
        context.session_name = self.filename_processor.extract_session_name(
            filename_without_ext,
            context.category
        )
...
class AudioraNexus:
    ...
    def process_all(
        ...
        audio_files = trackerator(audio_files, description="Processing audio", final_message="All audio processed!") if progress_bar else audio_files
